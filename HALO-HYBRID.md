...
| `GGML_CUDA_MMQ_MOE_J_FACTOR` (default 2, `0` restores upstream) | MMQ picks the column tile `J` that minimizes the tile count for `ncols_max`, which for MUL_MAT_ID is the whole ubatch, so the expert GEMMs always ran `J = 128` while an average expert has 20 tokens at `-ub 1024` (10 of 512 experts per token): 15–30% of each tile's columns were live and most grid blocks (row tiles × ubatch/J × 512 experts) exited empty. The ids path now sizes `J` for `factor × expected tokens per expert` (`ncols_hint`); the grid still covers every column | warm two-lane prefill, hybrid-12, 3.7K / 15K / 30K prompts: `-ub 2048` 1178 / 1036 / 853 → **1368 / 1152 / 935**, `-ub 1024` 891 / 832 / 723 → **1231 / 1058 / 886** (+16–38%); factor 3 and 4 are worse (4 collapses `-ub 1024` to 693 / 709 / 589); output identical |
