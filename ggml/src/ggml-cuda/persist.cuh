...
//
// GGML_CUDA_PERSIST=1 enables (off by default); GGML_CUDA_PERSIST_MIN=<n> is the minimum region length (default 2;
// test-backend-ops uses 1 so every op body is compared against the CPU on its own).
//
