...
static void launch_gated_delta_net(
        ...
        const int32_t * state_idx, int64_t state_row_stride, cudaStream_t stream) {
...
#define GDN_LAUNCH(S)                                                                                        \
    if (state_idx != nullptr) {                                                                              \
        ggml_cuda_kernel_launch(gated_delta_net_cuda<S, KDA, keep_rs_t, true>, launch_params,                \
            ...
            sb1, sb2, sb3, neqk1_magic, rq3_magic, scale, state_slot_stride, K, state_idx, state_row_stride); \
    } else {                                                                                                 \
    ...
    if (kda) {
        if (keep_rs) {
            launch_gated_delta_net<true, true>(q_d, k_d, v_d, g_d, b_d, s_d, dst_d, state_d,
                ...
                sb1, sb2, sb3, neqk1, rq3, scale, state_slot_stride, K, state_idx, state_row_stride, stream);
        } else {
